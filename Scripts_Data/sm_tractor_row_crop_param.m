...
VehicleData.Body.RA = -1.3155-1.2575;              % m
...
VehicleData.Body.Ref2Steer.p = [-1.5575 0.0145 2]; % m
...
VehicleData.SuspF.Track           = 0.9355*2;    % m tractorsk.chassis.fa.wctr.y
...
VehicleData.TireDataF.ptcld_cyl   = Point_Cloud_Data_Cylinder(...
    ...
    VehicleData.TireDataF.param.DIMENSION.WIDTH,nptsCylRear,false);

% Generate tread-aligned point cloud for front tire
% Save within data structure
[ptcld_loc,ptcld_hull] = sm_tractor_row_crop_ptcld_tiretread('Wheel_FL_Tire.stl',[2 3],0.52,[3 -2 1]);
VehicleData.TireDataF.ptcld_tread = ptcld_hull;
VehicleData.TireDataF.ptcld_treadLoc = ptcld_loc;
...
% Separation of wheels on this axle
VehicleData.SuspR.Track            = 0.9195*2;    % m  %tractorsk.chassis.ra.wctr.y/2
...
% Save within data structure
[ptcld_loc,ptcld_hull] = sm_tractor_row_crop_ptcld_tiretread('Wheel_RL_Tire.stl',[2 3],0.68,[3 -2 1]);

VehicleData.TireDataR.ptcld_tread = ptcld_hull;
VehicleData.TireDataR.ptcld_treadLoc = ptcld_loc;

% Select point cloud for use in model
% Can swap by copying in other data set
VehicleData.TireDataR.ptcld = VehicleData.TireDataR.ptcld_tread;

VehicleData.SuspR.Hub.Height   = VehicleData.TireDataR.param.DIMENSION.UNLOADED_RADIUS; % m
